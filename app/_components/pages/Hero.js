...
function Hero() {
  return (
    <section
      className="min-h-screen h-full flex flex-col top-full pt-32 relative overflow-hidden"
      id="home"
      aria-label="Strategic Business Consulting"
    >
      <svg
        width="678"
        height="635"
        ...
        className="hidden md:block absolute md:-right-15 md:top-4 lg:right-10 lg:top-25 xl:right-50  xl:top-10 z-0"
      >
        <path
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-4 md:px-12 lg:mx-auto z-1">
        <div className="space-y-4 md:max-w-xl text-center md:text-start pb-4">
          <p className="text-text-muted text-xl">
            YOUR NAME, <br />
            BUSINESS CONSULTING
          </p>
          <h1 className="text-4xl md:text-5xl font-semibold max-w-xl">
            Consulting for Business Growth
          </h1>
          <p className="md:text-lg lg:text-xl text-text-muted max-w-xl">
            I help founders and teams scale efficiently with focused strategy,
            streamlined operations, and results-driven execution.
          </p>
          <div
            className="flex flex-col md:flex-row justify-start items-center gap-4 md:gap-8"
            aria-label="Primary Actions"
          >
            <Link
              href="#contact"
              className="px-4 py-2 text-text-secondary bg-accent hover:-translate-y-1 hover:bg-accent-hover transition text-lg md:text-xl font-semibold"
            ...
            <Link
              href="#process"
              className="px-4 py-2 border hover:-translate-y-1 transition text-lg md:text-xl"
              aria-label="Learn how I work"
            >
              How I Work
            </Link>
          </div>
      ...
      <ul
        className="mt-8 md:mt-auto px-8 py-4 md:px-16 md:py-8 grid grid-cols-1 md:grid-cols-3 gap-8 bg-white md:h-46 lg:h-44 z-1"
        aria-label="Key business stats"
      >
        {brandLogos.map((brand) => (
          <li
            className="relative w-48 h-full m-auto flex items-center justify-center"
            key={brand.name}
          >
            <Image
              src={brand.logo}
              alt={`Collaborated with ${brand.name}`}
              fill
